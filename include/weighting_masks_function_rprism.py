...
def xiMasks(xi_0, w_xi):
    ...
    left_of_masks = [-14.2, -8.3]  # left most limit of each mask in order, on inital xi position
    right_of_masks = [-13, -6.4]  # right most limit of each mask in order, on initial xi position

    print(f'Left-most point is {xi_0[0]}, right-most is {xi_0[-1]}')
    # Apply masks to w_xi
...
def yMasks(y_0, w_y):
    ...
    top_of_masks = [1.0, 0.02]#[1.0, 0.58, 0.36]  #upper limit of each mask in order, on inital y position
    bot_of_masks = [0.1,-1.0]#[0.69, 0.52, -1.0]  #lower limit of each mask in order, on inital y position   
