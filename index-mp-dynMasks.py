# index-mp-multmasks.py can be used to propagate objects after simulation through fields in main.py
...
# Special masking options to use if any of the previous options are set to True; set false if only stationary masks are desired:
# Set only one of the following as TRUE at one time, keep the rest FALSE
useCurtainMask_y = True#False                   #Create a mask where only one side moves continuously
useMovingMask_y = False                    #Create a mask that continuously moves
useMovingBand_y = False                    #Create a thin band of electrons that moves continuously (inverse of moving mask)
...
dy_mask = 0.0                                # When using curtain masking, these step variables are 
dz_mask = 0.0                                # When using moving masks, these variables give the initial vertical/longitudinal edge of the mask 
                                             # When using moving bands, these variables give the initial width of the mask (e.g. band will initially stretch from r to r-dr)

mask_step = 0.1                              #General step size variable used for specialty masks
nMasks = 2                              #Variable for number of distinct masks to create when using continuously changing masks; MAKE SURE TO SET IT TO ONE IF NEITHER OF THE CONTINUOUS MASKS ARE USED
...
if __name__ == '__main__':
    ...
    numberOfCores = 6 #mp.cpu_count()//2
    ...
    if (len(sys.argv) >= 2):
        ...
        if ((useCurtainMask_y and useMovingMask_y) or (useCurtainMask_z and useMovingMask_z)):
            assert (useCurtainMask_y != useMovingMask_y) and (useCurtainMask_z != useMovingMask_z), "Functionality not implemented yet. Use only one dynamic masking procedure at a time!"
        
        if ((useCurtainMask_y and useCurtainMask_z) or (useMovingMask_y and useMovingMask_z)):
            assert (useCurtainMask_y != useCurtainMask_z) and (useMovingMask_y != useMovingMask_z), "Cannot use dynamic masking procedures for different directions at a time!"
        
        print(not(useCurtainMask_y and useMovingMask_y and useMovingBand_y))
            
        if (useCurtainMask_y == False and useMovingMask_y == False and useMovingBand_y == False) and (useCurtainMask_z == False and useMovingMask_z == False and useMovingBand_z == False):
            assert nMasks == 1, "Don't do more loops than necessary!"
        ...
        # WEIGHTING IMPORTS/SAVING
            ...
            if (skipWeightingCalc):
                ...
                # topM and bottomM are only used for plotting purposes
            ...
            if (useCurtainMask_y or useMovingMask_y or useMovingBand_y):
                print(f'Iteration number {N+1} complete.')
                dy_mask += mask_step
            if (useCurtainMask_z or useMovingMask_z or useMovingBand_z):
                print(f'Iteration number {N+1} complete.')
                dz_mask += mask_step
